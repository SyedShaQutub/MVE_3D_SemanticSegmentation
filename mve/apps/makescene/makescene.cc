...
import_images (AppSettings const& conf)
{
    ...
    util::fs::Directory dir;
    try { dir.scan(conf.input_path, true);}
    catch (std::exception& e)
    {
    ...
    util::fs::Directory dir_seman;
    try { dir_seman.scan(conf.input_path_seman, true);}
    catch (std::exception& e)
    {
        std::cerr << "Error scanning input dir of semantic info: " << e.what() << std::endl;
        std::exit(EXIT_FAILURE);
    }

    if(dir.size() == dir_seman.size()){
        std::cout<<"\n original images and the corresponding semantic images are found \n";
    }
    else{
        std::cout<<"\n Terminating program: some semantic images corresponding to original images are not found \n";
        return;
    }
    ...
    std::sort(dir_seman.begin(), dir_seman.end());
...
#pragma omp critical
        ...
        std::string fname_seman = dir_seman[i].name;
        std::string afname_seman = dir_seman[i].get_absolute_name();
        ...
        std::string exif_seman;
        mve::ImageBase::Ptr image_seman = load_any_image(afname_seman, &exif_seman);
        if (image_seman == nullptr)
            continue;
...
#pragma omp ordered
        ...
        /* Rescale and add semantic image. */
        image_seman = limit_image_size(image_seman, conf.max_pixels);
        if (orig_width == image_seman->width() && has_jpeg_extension(fname_seman))
            view->set_image_ref(afname_seman, "semantic");
        else
            view->set_image(image_seman, "semantic");
...
main (int argc, char** argv)
{
    ...
    // TODO @ssq
    // change it to input from command line
    std::string file ("/semantic_inference");
    conf.input_path_seman = conf.input_path+file;

    std::cout<<"\n\n conf.input_path_seman : "<<conf.input_path_seman<<"\n\n";
